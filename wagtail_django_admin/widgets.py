...
class TabularPermissionsWidget(FilteredSelectMultiple):
    ...
    def __init__(
        ...
        super(TabularPermissionsWidget, self).__init__(
    ...
    def render(self, name, value, attrs=None, renderer=None):
        ...
        for app in apps.get_app_configs():
            ...
            for model_name in app.models:
                ...
                view_perm_id = codename_id_map.get(
                    "%s_%s" % (view_perm_name, ct_id), False
                )
                ...
                if model._meta.permissions:
                    ...
                    for codename, perm_name in model._meta.permissions:
                        c_perm_id = codename_id_map.get(
                            "%s_%s" % (codename, ct_id), False
                        )
        ...
        VERSION = (2, 1, 0)
        old_django_has_view_perm = (
            True if "admin_view_permission" in settings.INSTALLED_APPS else False
        )
        if VERSION >= (2, 1, 0) and custom_permissions_available:
            ...
            if old_django_has_view_perm and custom_permissions_available:
                colspan = 7
            elif old_django_has_view_perm or custom_permissions_available:
                colspan = 6
            else:
                colspan = 5
