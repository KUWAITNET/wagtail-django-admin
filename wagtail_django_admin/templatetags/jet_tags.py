...
from wagtail_django_admin import settings, VERSION
from wagtail_django_admin.utils import (
...
@register.simple_tag()
def get_custom_jet_menu_configuration():
    menu = JetMenuConfiguration.objects.all()
    return menu


@assignment_tag(takes_context=True)
def jet_get_menu_from_dashboard(context):
    return get_menu_items_from_dashboard(context)


@register.simple_tag()
def menu_is_granted_for_user(request, menu):
    return request.user in menu.users.all() or not set(menu.groups.all()).isdisjoint(
        request.user.groups.all()
    )


@register.simple_tag()
def swap_url_lang(url):
    if "/en/" in url:
...
@register.filter
def replace_space_by_dash(value):
